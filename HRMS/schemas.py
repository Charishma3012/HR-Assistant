...
class MeetingBase(BaseModel):
    emp_ids: List[str] = Field(..., description="Employee identifiers invited to the meeting")
    start_dt: datetime = Field(..., description="Meeting start date and time")
    duration_minutes: int = Field(..., ge=1, description="Meeting duration in minutes")
...
class MeetingRead(MeetingBase):
    meeting_code: str = Field(..., description="Meeting code, e.g. 'M001'")
    status: str = Field(..., description="Meeting status")


class MeetingCancelRequest(BaseModel):
    meeting_code: str = Field(..., description="Meeting code to cancel")
